...
namespace SDK
{
	...
	public:
		...
		void ProcessEvent(UObject* UFunction, void* Params)
		{
			...
			{
				reinterpret_cast<void(__cdecl*)(UObject*, UObject*, void*)>(VTable[79])(this, UFunction, Params);
			}
		}
		...
		static UObject* StaticFindObject(UObject* Class, UObject* Outer, const wchar_t* Name, bool ExactClass)
		{
			...
			if (!StaticFindObject)
			{
				StaticFindObject = find_pattern<decltype(StaticFindObject)>(std::string("48 89 5C 24 ? 55 56 57 41 54 41 55 41 56 41 57 48 8D AC 24 ? ? ? ? 48 81 EC ? ? ? ? 48 8B 05 ? ? ? ? 48 33 C4 48 89 85 ? ? ? ? 48 83 FA"));
				// MessageBoxA(0, std::to_string(std::uintptr_t(StaticFindObject)).c_str(), "StaticFind", 0x40);
			}

			if (!StaticFindObject)
			{
				MessageBoxA(0, "cooked, your pc is slow.", 0, 0);
			}
		...
		static UObject* StaticLoadObject(void* ObjectClass, void* InOuter, const TCHAR* InName, const TCHAR* Filename, uint64_t LoadFlags, void* Sandbox, bool bAllowObjectReconciliation, void* InstancingContext)
		{
			...
			//if (!Address)
			//{
			//	auto Enc_Mask = Encrypt("xxxxxxxxxxxxxxxxx????xxx????xxx????xxxxxx????xxx????xx??xx");
			//	auto Enc_Pattern = Encrypt("\x40\x55\x53\x56\x57\x41\x54\x41\x55\x41\x56\x41\x57\x48\x8D\xAC\x24\x00\x00\x00\x00\x48\x81\xEC\x00\x00\x00\x00\x48\x8B\x05\x00\x00\x00\x00\x48\x33\xC4\x48\x89\x85\x00\x00\x00\x00\x48\x8B\x85\x00\x00\x00\x00\x45\x33\x00\x00\x8B\xBD");
			...
			if (!Address)
			{
				std::string signature = ("40 55 53 56 57 41 54 41 55 41 56 41 57 48 8D AC 24 ? ? ? ? 48 81 EC ? ? ? ? 48 8B 05 ? ? ? ? 48 33 C4 48 89 85 ? ? ? ? 48 8B 85 ? ? ? ? 45 33 ? ? 8B BD");
				Address = reinterpret_cast<void*>(find_pattern<uintptr_t>(signature)); // Specify template argument explicitly
			}
	...
	public:
		static uintptr_t Malloc(int Count, uint32 Alignment)
		{
			...
			if (!Address)
			{
				std::string signature = ("48 89 5C 24 ? 55 56 57 41 54 41 55 41 56 41 57 48 83 EC ? 4C 8B 25");
				Address = reinterpret_cast<void*>(find_pattern<uintptr_t>(signature));
			}
		...
		static void Free(void* Original)
		{
			...
			if (!Address)
			{
				std::string signature = ("48 85 C9 0F 84 ? ? ? ? ? ? ? 57 48 83 EC");
				Address = reinterpret_cast<void*>(find_pattern<uintptr_t>(signature));
			}
	...
	public:
		...
		{
			return *(USkeletalMeshComponent**)(this + 0x310);
		}
